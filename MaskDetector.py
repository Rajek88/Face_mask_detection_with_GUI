...
def run_on_video(Net, video_path, conf_thresh=1.0):
    ...
    while status:
        ...
        cv2.imshow('MaskDetection using Camera', img_raw[:,:,::-1])
        cv2.waitKey(1)
...
if __name__ == "__main__":
    ...
    if args.img_mode:
        ...
        cv2.namedWindow('MaskDetection with Camera', cv2.WINDOW_NORMAL)
        cv2.imshow('MaskDetection with Camera', result[:,:,::-1])
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    else:
